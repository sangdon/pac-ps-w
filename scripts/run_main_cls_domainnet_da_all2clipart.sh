...
SRC=DomainNetAll
TAR=DomainNetClipart
FEATNAME=All2Clipart
...
do
    # # upper bound
    # CUDA_VISIBLE_DEVICES=$GPU_ID python3 main_cls.py --exp_name exp_${EXPNAME}_desired_m_${M}_eps_${EPS}_delta_${DELTA}_expid_$i --data.src $TAR --data.tar $TAR --estimate --train_predset.method pac_predset_CP --data.seed None --model_predset.eps $EPS --model_predset.delta $DELTANOCAL --data.n_val_src $M --model.path_pretrained $MDLPATH 
    # # desired_eff
    # CUDA_VISIBLE_DEVICES=$GPU_ID python3 main_cls.py --exp_name exp_${EXPNAME}_desired_eff_m_${M}_eps_${EPS}_delta_${DELTA}_expid_$i --data.src $TAR --data.tar $TAR --estimate --train_predset.method pac_predset_CP --data.seed None --model_predset.eps $EPS --model_predset.delta $DELTANOCAL --data.n_val_src $MEFF --model.path_pretrained $MDLPATH --model_sd.path_pretrained $SDMDLPATH
    # iid algorithm
