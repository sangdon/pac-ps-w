...
def plot(args):
    ...
    for path, x, y in ds_tar.test:
        ...
        for p_i, ps_iid_i, ps_ours_i, c_iid_i, c_ours_i, y_i, yh_i in zip(path, ps_iid, ps_ours, ps_iid_membership, ps_ours_membership, y, yh):
            ...
            fn_img_src[-1] = fn_img_src[-1].replace('advex.feat', 'advex')
            fn_img_src = '/'.join(fn_img_src)
            img_src = pickle.load(open(fn_img_src, 'rb'))
            img_src = tc.tensor(img_src).byte()
            img_src = torchvision.transforms.ToPILImage()(img_src)
            ...
            img_src.save(fn_img_tar_iid)
            img_src.save(fn_img_tar_ours)
            
            #torchvision.utils.save_image(img_src, fp=fn_img_tar_iid)
            #torchvision.utils.save_image(img_src, fp=fn_img_tar_ours)
